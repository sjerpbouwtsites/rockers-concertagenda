...
${BEMify('event-block', [
  ...
  createDates(musicEvent) {
    ...
    let year;
    if (musicEvent.enlarged && musicEvent.start.substring(0, 4) === this.currentYear) {
      year = 'numeric';
    }
  ...
  createLocation(musicEvent) {
    ...
    return (
      <span>
        <span className="event-block__location-row">{locationObj.name}</span>
        <span className="event-block__location-row">{locationObj.city}</span>
      </span>
    );
  }
  ...
  createShortTextHTML(musicEvent, selectors) {
    ...
    return <p className={`${selectors.headerShortText}`}>{musicEvent.shortText}</p>;
  }
  ...
  async recursieveStijlEraf() {
    ...
    if (document.querySelector('[data-was-enlarged')) {
      ...
      window.scrollTo(
        0,
        wasEnlarged.offsetTop + document.getElementById('app-banner-top').clientHeight - 75,
      );
        ...
        setTimeout(() => {
          ...
          if (window.innerWidth > 1024) {
            const maxOffset = Math.max(
              Math.min(
                document.body.offsetHeight - thisElement.offsetHeight - 150,
  ...
  priceElement(musicEvent) { // eslint-disable-line
    // FIXME Naar eigen component
  ...
  createHideSoldOutBtn(musicEvent, selectors) {
    ...
    if (musicEvent.enlarged) return '';
    return (
      <button type="button" onClick={this.hideSoldOut} className={`${selectors.hideSoldOutBtn}`}>
  ...
  musicEventFilters() {
    ...
    const filtered = musicEvents
      ...
      .filter((musicEvent) => {
        ...
        return filterSettings.podia[musicEvent.location].checked ?? true;
      })
      .filter((musicEvent) => {
        if (!filterSettings?.daterange?.lower) {
          return true;
        }
        const lowerRangeTime = new Date(filterSettings.daterange.lower).getTime();
        const upperRangeTime = new Date(filterSettings.daterange.upper).getTime();
  ...
  bewerktMusicEventTitle(musicEvent) {
    ...
    if (toManyCapsInTitle) {
      // eslint-disable-next-line
      res.title = met[0].toUpperCase() + met.substring(1, 500).toLowerCase();
    } else {
      res.title = met;
    }
    ...
    if (res.title.length > 45) {
      res.title = res.title.replace(/\(.*\)/, '').replace(/\s{2,25}/, ' ');
    }
      ...
      >
        {
          filteredMusicEvents.map((musicEvent, musicEventKey) => {
            ...
            const firstOfMonthBlock = musicEvent.firstOfMonth ? (
              <time className="event-block__first-of-month-time">{musicEvent.eventMonth}</time>
            ) : (
              ''
            );
            ...
            const titelEnShortText = this.bewerktMusicEventTitle(musicEvent);
              ...
              <article
                ...
                onClick={
                  !musicEvent.enlarged && musicEvent.longText
                    ? this.loadLongerText.bind(this, musicEventKey)
                    : null
                }
              ...
              >
                ...
                <header className={selectors.header}>
                  <h2 className={selectors.headerH2}>
                    <span
                      className={selectors.headerEventTitle}
                      data-short-text={titelEnShortText.shortestText}
                    >
                  ...
                  <button type="button" onClick={this.sluitEnlarged} className={selectors.sluitEnlargedBtn}>
                ...
                <section className={selectors.main}>
                  ...
                  />
                  <footer className={selectors.footer}>{priceElement}</footer>
                </section>
                {hideSoldOutBtn}
              </article>
            );
