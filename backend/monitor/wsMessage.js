...
export default class wsMessage {
    ...
    initCheck() {
        if (typeof this.type !== "string") {
            throw new Error(
                `type moet string zijn, nu ${typeof this.type} ${this.type}`
            );
        }
        if (this.subtype && typeof this.subtype !== "string") {
            throw new Error(
                `subtype moet string zijn, nu ${typeof this.subtype} ${this.subtype}`
            );
        }
    ...
    setCheckType() {
        ...
        this.checkType["clients-html"] = (subtype) => {
            if (
                ...
                (subtype.length && subtype.length < 1)
            ) {
                this.throwSubtypeError();
            }
            return true;
        ...
        this.checkType["server-log"] = () => true;
        this.checkType["clients-log"] = () => true;
    }
