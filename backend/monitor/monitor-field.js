...
export default class MonitorField {
        ...
        if (Array.isArray(langeTekenMatches) && langeTekenMatches.length > 1) {
            langeTekenMatches.forEach((langeTeken) => {
                ...
                pretty = pretty.replace(
                    langeTeken,
                    `<span class='ingekort-lange-tekst' data-meer-tekst='${langeTeken}'>${kortereTekenReeds}</span>`
                );
            });
        }
        return pretty;
    ...
    get rollUpdatedHTML() {
        const listItems = this.data
            .map((rollRow) => {
                ...
                if (t) {
                    ...
                    t =
                        (rollRow.messageData?.content ??
                            rollRow.messageData) instanceof Object
        ...
        <span class='monitorfield__list-item-left'>${
            rollRow.messageData?.name ?? rollRow.messageData?.title ?? ""
        }</span>
                ...
                try {
                    ...
                    const bewerkteFoutTekst = rollRow.messageData.text
                        ...
                        .map((errorTextRow, index) => {
                            if (index < 1) return "";
                            if (errorTextRow.includes("node:internal"))
                                return "";
                            let t = errorTextRow;
    ...
    get tableUpdatedHTML() {
        ...
        Object.entries(this.data.workers).forEach(
            ([workerName, workerData]) => {
                ...
                workersPerFamily[family][workerIndex] = {
                    ...
                    amountOfEvents:
                        this.data[`amountOfEvents-${family}-${workerIndex}`]
                };
            }
        );
            ...
            .map((workerFamily, index) => {
                ...
                tableRowsFirstCellsTextcontent[index]
                    ...
                    .forEach((char, index) => {
                        ...
                        workerFamilyNamesCharsets[charsetIndex] =
                            workerFamilyNamesCharsets[charsetIndex] + char;
                    });
        ...
        ${sortedFamily
            .map((worker) => {
              ...
              ${worker.errors
                ...
                const numberHTML = Object.prototype.hasOwnProperty.call(
                    this.data,
