...
export default class HouseKeeping {
  ...
  init() {
    ...
    if (shell.removePublicEventImages) {
      const rbei = this.publicEventImagesCleanup(shell.removePublicEventImages);
    ...
    if (shell.removeSinglePageCache) {
      const rspc = this.singlePageCacheCleanup(shell.removeSinglePageCache);
  ...
  report() {
    ...
    console.log("");
    this.houseKeepingResults.forEach((hkr) => hkr.length && console.log(hkr));
  ...
  baseEventsCleanup(cleanInstructions) {
    ...
    cleanInstructions
      .filter((instr) => {
        return workerNames.find((wn) => wn.includes(instr));
      })
      .forEach((cleanBaseEventsOf) => {
        const bestand = bestaandeBaseEventLists.find((list) =>
          list.includes(cleanBaseEventsOf)
        );
        if (bestand) {
          fs.rmSync(`${fsDirections.baseEventlists}/${bestand}`);
          verwijderd.push(bestand);
        }
      });
    return verwijderd;
