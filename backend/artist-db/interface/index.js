...
async function laadStore() {
  return fetch('../store/allowed-artists.json').catch((err) => {
    console.error(err);
  }).then((r) => r.json());
}
...
async function jsonNaarTabelData() {
  ...
  const tabelData = Object.entries(jsonData).map(([key, values]) => {
    ...
    return [key, isSlug, spotifyId, metalEnc, genres, komendEvent, aanpassingsDatum];
  });
...
function exportData() {
  ...
  Array.from(document.querySelectorAll('tbody .ht__row_odd, tbody .ht__row_even'))
...
function initTabel(data) {
  ...
  const hot = new Handsontable(appContainer, {
    data,
    colHeaders: ['Key', 'Is Slug', 'Spotify ID', 'BS', 'Genres', 'Komend Event', 'Aanpassingsdatum'],
