...
const oudeEvents = JSON.parse(fs.readFileSync('../store/rock-artists-OUD.json'));
...
Object.entries(oudeEvents).forEach(([key, val]) => {
  ...
  nieuweEvents[kk] = [0, spotify, metalString, val?.genres ?? [], null, dateToday];
  let slug = slugify(kk);
  if (slug === kk) return;
  ...
  nieuweEvents[slug] = [1, spotify, metalString, val?.genres ?? [], null, dateToday]; 
});
