 ...
 * @returns [tekst, tekst]
 */
export function makeVerderScannen(
    toScan,
    reedsGevondenNamen,
        ...
        .map((scan) => {
            ...
            reedsGevondenNamen.forEach((rg) => {
                if (_scan.includes(rg)) {
                    _scan = _scan.replace(rg, "").replace(reg, "").trim();
                }
            });
...
function makeRefusedVar(isSlug, _eventDate, today, venueEventUrl) {
    return [isSlug ? 1 : 0, _eventDate, today, venueEventUrl];
}
// #endregion MAKE REFUSED VAR

// #region MAKE VOL ARTIST VAR
function makeVolArtistVar(
    isSlug,
    spotifyID,
    gevondenArtiest,
    _eventDate,
...
function genreAPIRespMap1(ga) {
    ...
    ga.genres = [...spotifyGenres, ...metalGenres, ...this._eventGenres].filter(
        (a) => a
    );
    return ga;
}
...
export async function harvestArtists(
    ...
    const reedsGevonden = toScan
        .map((scan) => this.scanTextForAllowedArtists(scan, ""))
        .filter((a) => Object.keys(a).length);
    const reedsGevondenRefused = toScan
        .map((scan) => this.scanTextForRefusedArtists(scan, ""))
        .filter((a) => Object.keys(a).length);
    ...
    const reedsGevondenRefusedNamen = reedsGevondenRefused
        .map((g) => Object.keys(g))
        .flat();
    ...
    if (!verderScannen.length) {
        this.consoleGroup(
            `Niets te vinden dat reeds bekende namen in hA2`,
    ...
    const potentieeleOverigeTitels = makePotentieeleOverigeTitels(
        settings,
        verderScannen
    );
    ...
    if (!potentieeleOverigeTitels.length) {
        this.consoleGroup(
            `Na tweede bew. niets verder gevonden om te onderzoeken hA3`,
            {
                title,
                bronTekst: toScan,
                naVerwijderenBekendeNamen: verderScannen
            },
            "harvestArtists",
            "fgmagenta"
        );
    ...
    const genreAPIResp = await this.recursiveAPICallForGenre(
        potentieeleOverigeTitels
    );
    ...
    genreAPIResp
        ...
        .filter((ga) => {
            const explGenreCheckRes = JSON.parse(
                this.checkExplicitEventCategories(ga.genres)
            );
            ...
            if (explGenreCheckRes.success === false) {
                this.refused[ga.title] = makeRefusedVar(
                    false,
                    eventDate,
                    this.today,
                    venueEventUrl
                ...
                if (ga.title !== ga.slug) {
                    this.refused[ga.slug] = makeRefusedVar(
                        true,
                        eventDate,
                        this.today,
                        venueEventUrl
    ...
    this.consoleGroup(
        `harvest artist debug bundel hA99`,
        {
