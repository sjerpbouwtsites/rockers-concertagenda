...
export default class Artists {
    ...
    getStoredArtist(artistName, slug) {
        ...
        const _b = Object.prototype.hasOwnProperty.call(
            this.allowedArtists,
            slug
        );
    ...
    getRecordsOfToday(recordType) {
        ...
        if (
            ["artists", "unclear", "allowedEvents", "refused"].includes(
                recordType
            )
        ) {
            return r;
    ...
    async getSpotifyConfirmation(title, slug, eventDate) {
        ...
        const ietsKutsDan = spotifyGenres.find((sg) =>
            this.terms.forbidden.find((forbiddenTerm) =>
                sg.includes(forbiddenTerm)
            )
        );
    ...
    async recursiveAPICallForGenre(lijst, resultaten = []) {
        ...
        const APICallsRes = await this.APICallsForGenre(
            deze.workTitle,
            deze.slug
        );
        resultaten.push({
    ...
    async APICallsForGenre(eventNameOfTitle, slug) {
        ...
        if (genreRes?.spotRes?.external_urls)
            delete genreRes.spotRes.external_urls;
    ...
    saveRefusedEvent(title, slug, eventDate) {
        const tt =
            title.length < this.minLengthLang ? title + eventDate : title;
        const ss = slug.length < this.minLengthLang ? slug + eventDate : slug;
    ...
    saveAllowedArtist(title, slug, spotify, metalEnc, genres, eventDate) {
        const tt =
            title.length < this.minLengthKort ? title + eventDate : title;
    ...
    persistNewRefusedAndRockArtists() {
        ...
        const unclearArtistsKeys = Object.keys(
            this.getRecordsOfToday("unclear")
        );
