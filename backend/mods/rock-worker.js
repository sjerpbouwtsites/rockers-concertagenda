...
export default class RockWorker extends Worker {
    ...
    static addWorkerMessageHandler(thisWorker, ArtistInst) {
        thisWorker.on("message", (message) => {
            ...
            RockWorker.monitorWebsocketServer.broadcast(
                JSON.stringify(parsedMessage, null, 2)
            );

            // DIT MOET HELEMAAL NAAR WORKER STATUS.
            if (
                parsedMessage.type === "process" &&
                parsedMessage?.subtype === "workers-status"
            ) {
                const statusString =
                    parsedMessage?.messageData?.content?.status ?? null;
                WorkerStatus.change(
            ...
            if (
                ...
                parsedMessage?.subtype === "scraper-results"
            ) {
                WorkerStatus.change(
                    thisWorker.workerName,
                    "todo",
                    parsedMessage?.messageData?.todo
                );
            }
            if (
                parsedMessage.type.includes("update") &&
                parsedMessage.subtype.includes("error")
            ) {
                WorkerStatus.processError(
                    thisWorker.workerName,
                    parsedMessage.messageData
                );
            }
