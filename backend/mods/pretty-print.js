...
function getOwnEnumPropSymbols(object) {
    return Object.getOwnPropertySymbols(object).filter((keySymbol) =>
...
function prettyPrint(input, options, pad) {
    ...
    if (Array.isArray(input)) {
        ...
        seen.push(input);
        var ret = `[${tokens.newLine}${input
            .map((el, i) => {
                const eol =
                    input.length - 1 === i
                        ? tokens.newLine
                        : `,${tokens.newLineOrSpace}`;
                let value = prettyPrint(
                    el,
                    combinedOptions,
                    pad + combinedOptions.indent
                );
    ...
    if (isObj(input)) {
        ...
        seen.push(input);
        var ret = `{${tokens.newLine}${objKeys_1
            .map((el, i) => {
                const eol =
