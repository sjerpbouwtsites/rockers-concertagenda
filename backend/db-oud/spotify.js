...
function searchArtistSearchForMetalOrRock(artists) {
    ...
    const rockOrMetal = artists.find((a) =>
        a.genres.find((g) => genresOK.find((gOk) => g.includes(gOk)))
    );
...
function createArtistsToCheckBatches(artistsNames, batchLength) {
    ...
    const artistToCheckBatchesLength = Math.ceil(
        artistsNames.length / batchLength
    );
...
async function spotifyLoopSearch(accessToken, spotifySettings) {
    ...
    for (
        let i = 0;
        i < artistNames.length && i < spotifySettings.maxArtistsLookup;
        i += 1
    ) {
    ...
    const artistsToCheckBatches = createArtistsToCheckBatches(
        artistsnamesToLookFor,
        spotifySettings.batchSize
    );

    const results = await recursiveArtistBatches(
        accessToken,
        artistsToCheckBatches,
        {}
    );
    const toPrint = {};
