function arrayCheckAndOrToString(event, key, required = true) {
    ...
    throw new Error(
        `geen array of string. typeof event.${key} ${typeof event[key]}`
    );
}
...
export function mapToStartDate(event, regexMode, months) {
    ...
    if (regexMode === "dag-maandNaam-jaar") {
        ...
        if (
            !Array.isArray(dateM) ||
            (Array.isArray(dateM) && dateM.length < 4)
        ) {
            event.errors.push({
...
export function mapToShortDate(event) {
    ...
    if (!b) {
        event.errors.push({
            error: new Error(
                `geen start of startDate beschikbaar in mapToShortDate`
            )
        });
        return event;
    }
