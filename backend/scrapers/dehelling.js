...
dehellingScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(
    ...
    ({ workerData }) =>
      Array.from(document.querySelectorAll(".c-event-card")).map((eventEl) => {
        const title =
          eventEl.querySelector(".c-event-card__title")?.textContent.trim() ??
          "";
        ...
        res.venueEventUrl = eventEl.querySelector(".c-event-card__link").href;
        ...
        const tijdenMatch =
          document
            ...
            ?.textContent.match(/(\d{1,2}:\d\d)\s?-\s?(\d{1,2}:\d\d)/) ?? [];
