...
dehellingScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(({ workerData }) => Array.from(
...
dehellingScraper.singlePage = async function ({ page, event }) {
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['.img--cover img', '.u-section__inner img'], mode: 'image-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.c-event-meta__table'],
  });
