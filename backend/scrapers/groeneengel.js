...
groeneEngelScraper.mainPage = async function () {
  ...
  let baseEvents = await page.evaluate(({ workerData, unavailabiltyTerms, months }) => Array
...
groeneEngelScraper.singlePage = async function ({ page, event }) {
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['.img-wrapper img'], mode: 'image-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.main-ticket-info'],
  });
