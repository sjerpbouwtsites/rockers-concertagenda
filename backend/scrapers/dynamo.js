...
scraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(
    ...
    ({ workerData }) =>
      Array.from(document.querySelectorAll(".group.image-border-trigger")).map(
        (baseEvent) => {
          ...
          res.shortText =
            baseEvent.querySelector("a + a div:last-child")?.textContent ?? "";

          res.soldOut = (baseEvent.querySelector("a + a")?.textContent ?? "")
            .toLowerCase()
