...
kavkaScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(
    ({ months, workerData, unavailabiltyTerms }) => Array.from(document.querySelectorAll('.events-list > a'))
      .filter((rawEvent) => Array.from(rawEvent.querySelectorAll('.tags'))
...
kavkaScraper.singlePage = async function ({ page, event }) {
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['img[src*="uploads"][src*="kavka"]', 'img[src*="kavka.be/wp-content"]'], mode: 'image-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.prijzen'],
  });
