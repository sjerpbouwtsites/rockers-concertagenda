  ...
  app: {
    ...
    mainPage: {
      ...
      skipEventRegexes: [
        /club\s?3voor12/im,
        /a\snew\stattoo/im,
        /boppin\W\s?the\s?rock/im,
        /bellen\s?blaas\s?festival/im,
        /wave\sinvasion/im,
      ],
    },
    singlePage: {
    ...
    ({ workerData, unavailabiltyTerms }) =>
      Array.from(document.querySelectorAll(".fusion-events-post")).map(
        (eventEl) => {
          ...
          res.venueEventUrl =
            eventEl.querySelector(".fusion-events-meta .url")?.href ?? null;
          ...
          res.mapToStartTime =
            datumTijdSplit.length > 1 ? datumTijdSplit[1].trim() : "20:00";
          res.mapToEndTime =
            eventEl.querySelector(".tribe-event-time")?.textContent.trim() ??
            "";
    ...
    () => {
      ...
      res.shortText =
        document
          ...
          ?.textContent.trim() ?? "";
      res.ticketURL =
        document.querySelector(".tribe-events-event-url a")?.href ?? null;
      if (!res.ticketURL) {
        res.price = "0";
      }
