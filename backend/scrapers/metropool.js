  ...
  let rawEvents = await page.evaluate(
    ...
    ({ workerData, unavailabiltyTerms }) =>
      Array.from(document.querySelectorAll(".paging-container")).map(
        (eventEl) => {
          const title =
            eventEl.querySelector(".event-title")?.textContent ?? null;
          const res = {
          ...
          res.shortText =
            eventEl.querySelector(".event-subtitle")?.textContent.trim() ?? "";
          const uaRex = new RegExp(unavailabiltyTerms.join("|"), "gi");
          res.unavailable = !!eventEl.textContent.match(uaRex);

          res.mapToStartDate =
            eventEl.querySelector(".event-date")?.textContent.trim() ?? "";
          res.mapToStartTime =
            eventEl.querySelector(".event-time")?.textContent.trim() ?? "";
