...
metropoolScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(({ workerData, unavailabiltyTerms }) => Array.from(document.querySelectorAll('.card--event'))
    .map((rawEvent) => {
...
metropoolScraper.singlePage = async function ({ page, event }) {
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['.object-fit-cover'], mode: 'image-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.doorPrice'],
  });
