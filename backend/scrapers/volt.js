...
scraper.mainPage = async function () {
  ...
  try {
    ...
    const noShows = await page.evaluate(
      () => document.querySelector(".view-empty")?.textContent ?? null
    );
    if (noShows) {
    ...
    this.handleError(
      error,
      "Volt wacht op laden eventlijst",
      "close-thread",
      null
    );
  }
  ...
  let rawEvents = await page.evaluate(
    ...
    ({ workerData, unavailabiltyTerms }) =>
      Array.from(document.querySelectorAll(".card-activity")).map(
        (rawEvent) => {
          const anchor =
            rawEvent.querySelector(".card-activity__title a") ?? null;
          ...
          res.shortText =
            rawEvent.querySelector(".card-activity__image-badges")
              ?.textContent ?? null;

          const uaRex = new RegExp(unavailabiltyTerms.join("|"), "gi");
          res.unavailable = !!rawEvent.textContent.match(uaRex);
          res.soldOut =
            rawEvent?.textContent.match(/uitverkocht|sold\s?out/i) ?? false;
          res.mapToStartDate =
            rawEvent
              .querySelector(".card-activity-list__date")
              ?.textContent.trim()
              .toLowerCase() ?? "";
          return res;
        }
...
scraper.singlePage = async function ({ page, url, event }) {
  ...
  const pageInfo = await page.evaluate(
    ...
    ({ event, url }) => {
      ...
      const tweedeTijdRij = document.querySelector(
        ".activity-info-row + .activity-info-row"
      );
      ...
      const endTimeM =
        tweedeTijdRij?.textContent.match(/\d\d\s?:\s?\d\d/) ?? null;
