...
doornroosjeScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(({ workerData, months }) => Array.from(document.querySelectorAll('.c-program__item'))
    .map((eventEl) => {
      const title =          eventEl.querySelector('.c-program__title')?.textContent.trim()
...
doornroosjeScraper.singlePage = async function ({ page, event }) {
  ...
  if (!event.venueEventUrl.includes('soulcrusher') && !event.venueEventUrl.includes('festival')) {
    ...
    pageInfo = await page.evaluate(({ event }) => {
      ...
      res.priceTextcontent = document.querySelector('.b-festival-content__container')?.textContent.trim() ?? '';
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['.c-header-event__image img', '#home img'], mode: 'image-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.c-btn__price', '.c-intro__col'],
  });
