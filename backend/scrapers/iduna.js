...
idunaScraper.mainPage = async function () {
  ...
  let metalEvents; let punkEvents; let
    doomEvents;
  try {
...
idunaScraper.singlePage = async function ({ page, event }) {
  ...
  const pageInfo = await page.evaluate(
    ({ months, event }) => {
      ...
      try {
        ...
        ).find((h2El) => h2El.textContent.toLowerCase().includes('deur'));
        if (doorEl) {
        ...
        const startEl = Array.from(
          document.querySelectorAll('#sideinfo h2'),
        ).find((h2El) => h2El.textContent.toLowerCase().includes('aanvang'));
        if (startEl) {
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ["#photoandinfo [style*='background']"], mode: 'background-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['#sideinfo'],
  });
