...
gebrdenobelScraper.mainPage = async function () {
  ...
  let punkMetalRawEvents = await page.evaluate(
    ({ workerData, unavailabiltyTerms }) => Array.from(document.querySelectorAll('.event-item'))
      .filter((eventEl) => {
  ...
  let rockRawEvents = await page.evaluate(
    ({ workerData }) => Array.from(document.querySelectorAll('.event-item'))
      .filter((eventEl) => {
        const tags =            eventEl.querySelector('.meta-tag')?.textContent.toLowerCase() ?? '';
...
gebrdenobelScraper.singlePage = async function ({ page, event }) {
  ...
  const pageInfo = await page.evaluate(
    ({ months, event }) => {
      ...
      const timeRow = eventDataRows.find(
        (row) => row.textContent.toLowerCase().includes('open')
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['.hero img'], mode: 'image-src',
  });
  ...
  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.gebrdenobel-price-manual', '.event-table'],
  });
