...
willemeeenScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(({ workerData, months, unavailabiltyTerms }) => Array.from(document.querySelectorAll('.we__agenda-item'))
    .map((rawEvent) => {
...
willemeeenScraper.singlePage = async function ({ page, url, event }) {
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['.main-img', '.we_program_text_image'], mode: 'image-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.ticket-col'],
  });
