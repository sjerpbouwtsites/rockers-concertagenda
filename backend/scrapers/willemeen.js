  ...
  let rawEvents = await page.evaluate(
    ...
    ({ workerData, months, unavailabiltyTerms }) =>
      Array.from(document.querySelectorAll(".we__agenda-item")).map(
        (rawEvent) => {
          const title =
            rawEvent.querySelector("[data-text]")?.getAttribute("data-text") ??
            "";
          const res = {
          ...
          res.shortText =
            rawEvent.querySelector(".we__agenda-item-genre")?.textContent ??
            null;
          res.startTime =
            rawEvent.querySelector(".we__agenda-item-info")?.textContent ??
            null;
          const dateText = rawEvent.getAttribute("date-text") ?? "";
          const dateM =
            dateText.match(/(?<day>\d\d)\s+(?<monthletters>\w+)/) ?? null;
