...
scraper.mainPage = async function () {
    ...
    let rawEvents = await page.evaluate(
        ...
        ({ workerData }) =>
            Array.from(
                document.querySelectorAll(".wpt_listing .wp_theatre_event")
            )
                .map((eventEl) => {
                    ...
                    const categorieTeksten = Array.from(
                        eventEl.querySelectorAll(
                            ".wpt_production_categories li"
                        )
                    ).map((li) => {
                        const categorieNaam = li.textContent
                            .toLowerCase()
                            .trim();
                ...
                .map(({ eventEl, categorieTeksten, venueEventUrl }) => {
                    ...
                    res.soldOut =
                        title.match(/uitverkocht|sold\s?out/i) ?? false;
                    ...
                    res.mapToStartDate =
                        eventEl
                            ...
                            .substring(3, 26) ?? "";
                    res.mapToStartTime =
                        eventEl.querySelector(".wp_theatre_event_starttime")
                            ?.textContent ?? "";
    ...
    rawEvents = rawEvents
        .map((event) =>
            mapToStartDate(event, "dag-maandNummer-jaar", this.months)
        )
        .map(mapToStartTime)
        .map(mapToShortDate)
