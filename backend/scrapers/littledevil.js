...
scraper.mainPage = async function () {
    ...
    let rawEvents = await page.evaluate(
        ...
        ({ workerData }) =>
            Array.from(
                document.querySelectorAll(".entry-content .bericht")
            ).map((eventEl) => {
                const venueEventUrl =
                    eventEl.querySelector(".titelbericht a").href ?? "";
                const title =
                    eventEl.querySelector(".titelbericht").textContent.trim() ??
                    "";
                const res = {
                    title,
                    errors: [],
                ...
                const ct =
                    eventEl.querySelector(".contentbericht")?.textContent ?? "";
                ...
                if (s.includes("gratis")) {
                    ...
                    try {
                        ...
                        res.errors.push({
                            error: new Error(
                                `price berekening met ${s} ${res.anker}`
                            )
                        });
                    }
                }
                ...
                if (startTimeM) {
                    res.mapToStartTime = startTimeM[0]
                        .replace(".", ":")
                        .padEnd(5, ":00");
                } else {
                ...
                res.soldOut =
                    eventEl.textContent.match(/uitverkocht|sold\s?out/i) ??
                    false;

                res.mapToStartDate =
                    eventEl
                        .querySelector(".einddatumbericht")
