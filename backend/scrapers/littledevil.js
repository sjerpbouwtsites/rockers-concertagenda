...
littledevilScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(({ workerData }) => Array
    ...
    .map((eventEl) => {
      ...
      const catTeksten = spl.filter((catT) => !catT.includes('tba') && !catT.includes('€') && !catT.includes(',-') && !catT.match(/\d\d/) && !catT.includes('gratis'))
        .map((a) => a.trim())
        .join(' - ');
