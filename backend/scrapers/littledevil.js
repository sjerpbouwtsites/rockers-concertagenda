...
const littledevilScraper = new AbstractScraper(makeScraperConfig({
...
littledevilScraper.mainPage = async function () {
  ...
  let rawEvents = await page.evaluate(({workerData}) => {
    ...
    return Array
      .from(document.querySelectorAll('.entry-content .bericht'))
      .map(eventEl => {
        const venueEventUrl = eventEl.querySelector('.titelbericht a').href ?? '';
        let title = eventEl.querySelector('.titelbericht').textContent.trim() ?? '';
        const res = {
          title,
          errors: [],
          venueEventUrl,
        ...
        const catTeksten = spl.filter(catT => {
          return !catT.includes("tba") && !catT.includes("€") && !catT.includes(',-') && !catT.match(/\d\d/) && !catT.includes('gratis');
        })
          .map(a => a.trim())
          .join(' - ')
        ...
        if (s.includes('gratis')) {
          ...
          try {
            const mmm = s.replace('-','00').replace(',','.').match(/(\d+)([,.]?)(\d{1,2})/);
            if (mmm){
        ...
        res.mapToStartDate = eventEl.querySelector('.einddatumbericht')?.textContent.toLowerCase() ?? '';
