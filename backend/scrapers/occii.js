...
import { mapToStartDate, mapToShortDate, mapToStartTime } from './gedeeld/datums.js';
...
const scraper = new AbstractScraper({
  ...
  app: {
    ...
    mainPage: {
      ...
      asyncCheckFuncs: ['refused', 'allowedEvent', 'forbiddenTerms', 'hasGoodTerms', 'hasAllowedArtist', 'success'],
    },
    singlePage: {
...
scraper.mainPage = async function () {
  ...
  await page.evaluate((months) => {
    document.querySelectorAll('.occii-event-times').forEach((eventTimeEl) => {
      ...
      Object.entries(months).forEach(([monthName, monthNumber]) => {
        if (eventTimeText.includes(monthName)) {
          ...
          const strippedT = eventTimeText.replace(monthName, '').trim();
  ...
  let rawEvents = await page.evaluate(
    ...
    ({ workerData, unavailabiltyTerms }) => {
      ...
      return Array.from(
        ...
        if (!oet || !oet.hasAttribute('data-month') || !oet.hasAttribute('data-day')) {
          res.corrupt = true;
          return res;
        }
