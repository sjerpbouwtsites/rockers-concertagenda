    ...
    const rawEvents = await fetch(this._s.mainPage.url)
        ...
        .then((fetchedData) => {
            ...
            const res = fetchedData?._embedded?.events.map((rawEvent) => {
                ...
                copyEvent.attractions = editAttractionsInRaw(
                    copyEvent?._embedded?.attractions ?? []
                );
                const hasVenues =
                    Array.isArray(copyEvent?._embedded?.venues) &&
                    copyEvent?._embedded?.venues.length;
                copyEvent.venue = hasVenues
...
ticketmasterScraper.singlePage = async function ({ event }) {
    ...
    if (event?.venue?.name) {
        pageInfo.location = event?.venue?.name
            ?.toLowerCase()
            .replace(/\W/g, "");
        ...
        if (event?.venue?.url) {
            const locatieMatch = event?.venue?.url.match(
                /venue\/([\w-]+)-tickets/
            );
    ...
    if (
        workerNames.includes(pageInfo.location) ||
        pageInfo.location === "metropoolenschede"
    ) {
    ...
    try {
        const priceR =
            event?.priceRanges.find((priceRange) =>
                priceRange.type.includes("fees")
            ) || event?.priceRanges[0];
        pageInfo.price =
            Object.prototype.hasOwnProperty.call(priceR, "max") && priceR.max;
    } catch (caughtError) {
    ...
    if (event.attractions.length > 1) {
        pageInfo.shortTitle = event.attractions.reduce((prev, next) => {
            const isMetal = ticketmasterScraper.classificationsMetal(
                next.classifications[0]
            );
            if (isMetal) {
        ...
        pageInfo.shortTitle = pageInfo.shortTitle.substring(
            0,
            pageInfo.shortTitle.length - 2
        );
    }
    ...
    await this.downloadImageCompress(
        event,
        event.image,
        imagePath,
        pageInfo.location
