...
scraper.mainPage = async function () {
    ...
    if (cookiesNodig) {
        await page.evaluate(() => {
            ...
            document
                .querySelector(".cookie__settings .cookie__process")
                .click();
        });
        await this.waitTime(50);
    }
    ...
    let rawEvents = await page.evaluate(
        ...
        ({ workerData, unavailabiltyTerms }) =>
            Array.from(
                document.querySelectorAll(".program__list .program__item")
            )
                .filter((eventEl) => !eventEl.classList.contains("is-hidden"))
                .map((eventEl) => {
                    const title =
                        eventEl.querySelector(".program__content h2")
                            .textContent ?? "";
                    ...
                    res.soldOut =
                        ...
                        false;
                    res.longText =
                        eventEl.querySelector(".program__content")
                            ?.textContent ?? null; // tijdelijk om in te controleren
                    return res;
                }),
        {
            workerData,
