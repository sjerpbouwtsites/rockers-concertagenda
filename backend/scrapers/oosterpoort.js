...
oostpoortScraper.mainPage = async function () {
  ...
  const cookiesNodig = await page.evaluate(() => document.querySelector('html').classList.contains('has-open-cookie'));
  ...
  let rawEvents = await page.evaluate(({ workerData, unavailabiltyTerms }) => Array
    .from(
...
oostpoortScraper.singlePage = async function ({ page, event }) {
  ...
  const imageRes = await this.getImage({
    page, event, pageInfo, selectors: ['.hero__image', '.festival__header__image'], mode: 'image-src',
  });
  pageInfo.errors = pageInfo.errors.concat(imageRes.errors);
  pageInfo.image = imageRes.image;

  const priceRes = await this.getPriceFromHTML({
    page, event, pageInfo, selectors: ['.event__pricing__costs', '.festival__tickets__toggle'],
  });
